...
const addABrand = async (event) => {
  ...
  const { data: responseFromBrandAdded, error } = await useFetch("/api/proxy/admin/brands",
    {
      method: "POST",
      body,
      ...
      <div class="w-full">
        ...
        >
          ...
          <Button
            ...
            :disabled="
              !brandToAdd.brandName ||
              !brandToAdd.brandMetaTitle ||
              !brandToAdd.brandMetaDescription ||
              !fileToUp ||
              !brandToAdd.brandVisibilityStatus.code
            "
            label="Add Brand"
            type="submit"
          />
